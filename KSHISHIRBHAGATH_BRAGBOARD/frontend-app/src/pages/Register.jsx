...
// --- SVG ICONS ---
// These are needed for the visuals on the registration form
const BragboardIcon = () => (
...
// --- AUTH LAYOUT (shared with login/register) ---
        ...
        <AuthLayout title="Create Your Account">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="relative">
                    ...
                    <input type="text" placeholder="Full Name" value={name} onChange={e => setName(e.target.value)} required className="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition" />
                </div>
                <div className="relative">
                    <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
            ...
            <p className="text-center text-sm text-gray-600 mt-4">
                Already have an account? <button onClick={onNavigateToLogin} className="font-medium text-indigo-600 hover:text-indigo-500">Sign in</button>
            </p>
        </AuthLayout>
    );
}
...
export default function RegisterWrapper() {
    ...
    const mockNavigateToLogin = () => console.log('Navigating to login (mocked)');
    const mockRegister = (email, password, name) => console.log(`Attempting registration with: ${email}, ${name} (mocked)`);
